...
export function VerifyPage() {
 ...
 const handleAnalyze = async () => {
  ...
  const doc = new jsPDF();
  doc.setFillColor(25, 33, 52);
  doc.rect(0, 0, 210, 28, "F");

  doc.setFillColor(52, 152, 219);
  doc.circle(28, 15, 5, "F");

  doc.setTextColor(255, 255, 255);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(9);
  doc.text("TL", 25.2, 16.8);

  doc.setFont("helvetica", "bold");
  doc.setFontSize(22);
  doc.text("TruthLens AI", 40, 16);

  doc.setFontSize(11);
  doc.text("Deepfake Verification Report", 40, 23);

  doc.setTextColor(0, 0, 0);
  const reportId = `TL-${Date.now()}`;
  const generatedOn = new Date().toLocaleString();

  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  doc.text(`Report ID : ${reportId}`, 20, 38);
  doc.text(`Generated : ${generatedOn}`, 20, 45);
  doc.line(20, 50, 190, 50);
  // Media Information
  doc.setFont("helvetica", "bold");
  doc.setFontSize(13);
  doc.setTextColor(40, 40, 40);
  doc.text("Media Information", 20, 62);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);

  doc.text(`File Name : ${file?.name || "Unknown"}`, 20, 72);

  doc.text(
    `File Type : ${mediaType.charAt(0).toUpperCase() + mediaType.slice(1)}`,
    20,
    80
  );
  ...
  doc.text(`File Size : ${fileSizeMB} MB`, 20, 88);
  ...
  doc.roundedRect(15, 102, 180, 135, 3, 3, "FD");

  // ==========================
  // Verdict Banner
  // ==========================
  doc.setFillColor(
    result.verdict === "success" ? 34 : 220,
    result.verdict === "success" ? 197 : 53,
    result.verdict === "success" ? 94 : 69
  );

  doc.roundedRect(20, 108, 170, 18, 3, 3, "F");
  ...
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.text("Confidence Score", 25, 138);

  const confidence = Number(result.confidence);

  doc.setFillColor(230, 230, 230);
  doc.roundedRect(25, 143, 120, 6, 2, 2, "F");

  doc.setFillColor(
    result.verdict === "success" ? 34 : 239,
    result.verdict === "success" ? 197 : 68,
    result.verdict === "success" ? 94 : 68
  );
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(10);
  doc.text(`${confidence.toFixed(2)}%`, 152, 148);
  ...
  y += 10;
  doc.text(`Processing Time : ${result.processedIn}`,25,y);

  if(mediaType==="video"){

    y += 10;
    doc.text(`Analyzed Frames : ${result.analyzedFrames}`,25,y);

    y += 10;
    doc.text(`Real Frames : ${result.realFrames}`,25,y);

    y += 10;
    doc.text(`Fake Frames : ${result.fakeFrames}`,25,y);

    y += 10;
    doc.text(`Total Frames : ${result.totalFrames}`,25,y);
  ...
  doc.text("Generated by TruthLens AI", 15, 286);
  doc.text("This report is AI-generated and intended for informational purposes only.", 15, 291);
  doc.text("© 2026 TruthLens AI", 15, 296);
